...
function LandingSection() {
	...
	const [ghost3Pos, setGhost3Pos] = useState({ x: 0, y: 0 });
	...
	useEffect(() => {
		...
		const maxOffsetX = (sectionWidth - ghostSize);
		const maxOffsetY = (sectionHeight - ghostSize);
		...
		console.log(`sectionWidth: ${sectionWidth}, sectionHeight ${sectionHeight}`);
	
		const interval1 = setInterval(() => {
			const offset = getRandomOffset();
			console.log(`Ghost 1 - x:${offset.x}, y:${offset.y}`);
			setGhost1Pos(offset);
		}, 1500);
	...
	return (<>
		<div id="landing">
			<div className='container m-auto w-full relative h-96 py-96 md:h-svh md:py-0'>
				<Ghost ghostPos={ghost1Pos}/>
				<Ghost ghostPos={ghost2Pos}/>
